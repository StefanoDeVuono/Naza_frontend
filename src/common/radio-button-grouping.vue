<template>
  <div class="radio-button-grouping">
    <div class="header">
      <h2>{{ name }}</h2>
      <div class="supplemental-price" v-show="supplementalPrice">+ {{ supplementalPrice }}</div>
    </div>
  ...
  computed: {
    localValue: {
      ...
      set(x) {
        ...
        if (optionValue && optionValue.supplemental_price && optionValue.supplemental_price > 0) {
