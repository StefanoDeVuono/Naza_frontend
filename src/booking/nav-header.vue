  ...
  computed: {
    ...
    formattedHours: function() {
      if (this.totalDuration <= 1) {
        return `${this.totalDuration} HR`
      } else {
        return `${this.totalDuration} HRS`
      }
    }
  },
...
<style lang="less">
  ...
  .material-design-icon.keyboard-backspace-icon, .material-design-icon.keyboard-backspace-icon > .material-design-icon__svg {
    height: 30px;
    width: 30px;
  }
  ...
  .booking-nav-header {
    ...
    .stats {
      ...
      #total-cost-value, #total-duration-value {
        margin-left: 0.25em;
      }
    }
  }
